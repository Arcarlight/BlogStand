...
全站换成 **VonwaonBitmap 12px** 点阵字体。开关键在 `hugo.toml` 的
`pixelFont`，改 `false` 一行就回到原来的微软雅黑。

字体放在 `static/fonts/vonwaon-12px.woff2`（**94 KB**，从原版 1.5 MB 的 TTF
按全站实际用到的 2212 个字符子集化而来），授权是
`Created by Haoyu Qiu. No rights reserved.`，可自由使用与嵌入。

**为什么必须"整倍缩放"**：这份字体是「轮廓即像素方块」的矢量字体，
`unitsPerEm = 1200`，每个设计像素正好 **100 单位**（已全量核对 3000 个字形，
没有任何一个坐标落在网格外，且全是直线、没有曲线）。所以它锐利的条件是
...
1. **不能加粗。** 这份字体没有粗体，浏览器合成的假粗体会把方块糊掉
   （实测中间灰像素占比从 39% 涨到 52%）。所以 CSS 里把 `font-weight`
   统一压成 `normal`，层级**改用字号**表达（`--px` / `--px-lg` / `--px-xl`），
   以及反白、色块这些不依赖粗细的手段。页脚那个 `<strong>` 是用黄底红字代替加粗的。
2. **字号不能写死。** 任何地方要用点阵字体，字号都必须取自
   `var(--px)` / `var(--px-lg)` / `var(--px-xl)` / `var(--px-sm)`。
   写死一个别的数字（比如 13px）就会掉出整数倍，方块立刻变糊。
...
抗锯齿 —— 竖排的方块是严丝合缝的 2 设备像素、一个灰行都没有，但横排大约
每 6 个设计像素会漂半个设备像素，于是边缘带一点灰和 LCD 彩边。想要 100%
硬边，得再叠一层二值化滤镜：
