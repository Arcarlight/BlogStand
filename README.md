...
| 整站回到宋体（最彻底的"年代化"） | `hugo.toml` 的 `fontStack` 换成 `SimSun,"宋体",sans-serif` |

> **为什么没有默认用宋体**：站长试过。他的屏幕是 150% 缩放，宋体在非整数
> 缩放下会走轮廓渲染、发灰发虚，可读性反而变差，所以主动回退了。宋体的
> 点阵感只在 12~17 设备像素区间才出得来，150% 缩放下 12px 会变成 18 设备
> 像素，正好错过。所以这条留成可选项。
